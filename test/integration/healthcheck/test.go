...
/**
	Overview
		- Tests the health checks of the extension: provider-azure.
...
var _ = ginkgo.Describe("Provider-azure integration test: health checks", func() {
	...
	ginkgo.Context("ControlPlane", func() {
		...
		ginkgo.Context("Condition type: ShootControlPlaneHealthy", func() {
			f.Serial().Release().CIt(fmt.Sprintf("ControlPlane CRD should contain unhealthy condition because the deployment '%s' cannot be found in the shoot namespace in the seed", azure.CloudControllerManagerName), func(ctx context.Context) {
				err := healthcheckoperation.ControlPlaneHealthCheckDeleteSeedDeployment(ctx, f, f.Shoot.GetName(), azure.CloudControllerManagerName, gardencorev1beta1.ShootControlPlaneHealthy)
