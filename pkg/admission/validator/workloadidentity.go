...
func (wi *workloadIdentity) Validate(_ context.Context, newObj, oldObj client.Object) error {
	...
	if oldObj != nil {
		...
		errList := azurevalidation.ValidateWorkloadIdentityConfigUpdate(oldConfig, newConfig, fieldPath)
		if len(errList) > 0 {
	...
	errList := azurevalidation.ValidateWorkloadIdentityConfig(newConfig, fieldPath)
	if len(errList) > 0 {
		return fmt.Errorf("validation of target system's configuration failed: %w", errList.ToAggregate())
	}
	return nil
