...
func (cp *cloudProfileValidator) Validate(_ context.Context, newObj, _ client.Object) error {
	...
	if err := azurevalidation.RestrictToArchitectureAndNetworkingCapability(capabilityDefinitions, field.NewPath("spec").Child("machineCapabilities")); err != nil {
		return err
	}

	return azurevalidation.ValidateCloudProfileConfig(cpConfig, cloudProfile.Spec.MachineImages, capabilityDefinitions, providerConfigPath).ToAggregate()
}
