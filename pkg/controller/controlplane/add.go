...
var (
	...
	logger = log.Log.WithName("azure-controlplane-controller")
)

// AddOptions are options to apply when adding the Azure controlplane controller to the manager.
...
func AddToManagerWithOptions(mgr manager.Manager, opts AddOptions) error {
	return controlplane.Add(mgr, controlplane.AddArgs{
		Actuator: NewActuator(genericactuator.NewActuator(azure.Name,
			getSecretConfigsFuncs(opts.UseTokenRequestor), getShootAccessSecretsFunc(opts.UseTokenRequestor), getLegacySecretNamesToCleanup(opts.UseTokenRequestor, legacySecretNamesToCleanup), nil, nil, nil,
