...
import (
	...
	azinfra "github.com/gardener/gardener-extension-provider-azure/pkg/controller/infrastructure"
...
var _ = Describe("Flowreconciler", func() {
	Context("with flow annotation in infrastruture", func() {
		...
		metav1.SetMetaDataAnnotation(&infra.ObjectMeta, azinfra.AnnotationKeyUseFlow, "true")
	...
	Context("with flow annotation in shoot", func() {
		cluster := internalinfra.MakeCluster("11.0.0.0/16", "12.0.0.0/16", "europe", 1, 1)
		metav1.SetMetaDataAnnotation(&cluster.Shoot.ObjectMeta, azinfra.AnnotationKeyUseFlow, "true")
		It("should use the FlowReconciler", func() {
