...
func (e *ensurer) EnsureCloudProviderSecret(ctx context.Context, _ gcontext.GardenContext, new, _ *corev1.Secret) error {
	if new.ObjectMeta.Labels != nil && new.ObjectMeta.Labels[securityv1alpha1constants.LabelWorkloadIdentityProvider] == "azure" {
		...
		new.Data[azure.ClientIDKey] = []byte(workloadIdentityConfig.ClientID)
		new.Data[azure.TenantIDKey] = []byte(workloadIdentityConfig.TenantID)
		new.Data[azure.SubscriptionIDKey] = []byte(workloadIdentityConfig.SubscriptionID)
		new.Data[azure.WorkloadIdentityTokenFileKey] = []byte(azure.WorkloadIdentityMountPath + "/token")
		return nil
	}
